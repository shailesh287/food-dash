...
import { selectItemsInCart, selectTotalPrice } from "../Redux/cartSlice";

const OrderSummary = () => {
  const cartItems = useSelector(selectItemsInCart);
  const totalPrice = useSelector(selectTotalPrice);
  const discount = (totalPrice * 0.15) / 100;
  ...
  return (
    <div className="basis-5/12 h-fit sticky top-40 p-8 rounded-md border shadow-md my-8 md:m-0">
      ...
      <div className="py-4 text-lg space-y-4 border-b">
        ...
        <div className="flex justify-between items-center font-semibold">
          <p className="font-normal">Discount (15 %)</p>
      ...
      <div className="flex justify-between items-center font-bold text-lg md:text-2xl py-4 border-b">
        <h1>Total Amount</h1>
        <h1 className="text-orange-500">₹ {parseFloat(totalAmt).toFixed(2)}</h1>
      </div>
